...
#define TIME_TO_WAIT CLOCKS_PER_SEC * 0.1
void
just_record (x_conn_t *conn, char *filename) {
	...
	for (;;) {
		...
		wait_time = (50 - (clock() - time) / CLOCKS_PER_SEC * 1000);
		if (wait_time > 0) usleep(wait_time * 1000);
	}
	printf("Frame count: %d", ost->frame_pts);
